...
import { AgentToolDefinition, AgentExecutionContext, AgentExecutionResult, AgentIdentity, AgentStreamEventType } from './agent-engine.types';
...
export abstract class AgentEngine {
  ...
  /**
   * Streaming variant of execute(). Runs tool-use iterations synchronously
   * (emitting tool_start/tool_result events), then streams final response tokens.
   */
  async executeStream(
    ...
    try {
      while (iterations < MAX_AGENT_ITERATIONS) {
        ...
        // On each iteration we don't know in advance whether it'll end in tool_use or end_turn.
        // Use complete() for all iterations; switch to stream() only when we detect end_turn
        // by first completing, then if stop_reason is end_turn re-stream the response.
        // Simpler: use aiProvider.stream() and collect tool calls; emit tokens for text deltas.
        let collectedTokens = '';
        const response = await this.aiProvider.stream(
          {
            model,
            system: systemPrompt,
            messages,
          ...
          (event) => {
            if (event.type === 'text_delta' && event.delta) {
              collectedTokens += event.delta;
              onEvent({ type: 'token', delta: event.delta });
        ...
        if (response.stopReason === 'end_turn') {
          finalResponse = collectedTokens ||
            (() => {
              const tb = assistantMessage.content.find((b) => b.type === 'text');
              return tb ? (tb as { type: 'text'; text: string }).text : '';
            })();
          break;
        }
        ...
        if (response.stopReason === 'tool_use') {
          ...
          for (const block of assistantMessage.content) {
            ...
            onEvent({ type: 'tool_start', toolName: block.name, input: block.input });
            const toolStart = Date.now();
            ...
            if (approvalResult.outcome === 'PENDING') {
              pendingApprovalId = approvalResult.approvalRequestId;
              finalResponse = `Action "${block.name}" requires human approval. Approval request ID: ${pendingApprovalId}`;
              const traceResult = await this.tracer.finalizeTrace(traceCtx, {
                totalInputTokens, totalOutputTokens, iterations, toolCalls: toolCallRecords, finalStatus: 'PENDING_APPROVAL',
              });
              const result = { response: finalResponse, traceResult, pendingApprovalId };
              onEvent({ type: 'agent_done', result });
              return result;
            }

            if (approvalResult.outcome === 'DENIED') {
              onEvent({ type: 'tool_result', toolName: block.name, durationMs: Date.now() - toolStart, isError: true });
              toolResultBlocks.push({
                type: 'tool_result',
                toolUseId: block.id,
                content: `Permission denied: ${approvalResult.reason}`,
              ...
              toolCallRecords.push({
                toolCallId: block.id, toolName: block.name, input: block.input, output: null,
                isError: true, durationMs: Date.now() - toolStart, permissionLevel: toolDef.permissionLevel, wasApproved: false,
              });
              continue;
            }
            ...
            onEvent({ type: 'tool_result', toolName: block.name, durationMs, isError: toolIsError });
            toolCallRecords.push({
              toolCallId: block.id, toolName: block.name, input: block.input, output: toolOutput,
              isError: toolIsError, durationMs, permissionLevel: toolDef.permissionLevel, wasApproved: true,
            });
            toolResultBlocks.push({ type: 'tool_result', toolUseId: block.id, content: outputStr, isError: toolIsError });
          }
      ...
      const traceResult = await this.tracer.finalizeTrace(traceCtx, {
        totalInputTokens, totalOutputTokens, iterations, toolCalls: toolCallRecords, finalStatus: 'COMPLETED',
      });

      const result = { response: finalResponse, traceResult };
      onEvent({ type: 'agent_done', result });
      return result;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : String(err);
      this.logger.error(`Agent stream execution failed: ${errorMessage}`);
      onEvent({ type: 'agent_error', message: errorMessage });

      const traceResult = await this.tracer.finalizeTrace(traceCtx, {
        totalInputTokens, totalOutputTokens, iterations, toolCalls: toolCallRecords,
        finalStatus: 'FAILED', errorMessage,
      });
